    ...
    <div ref={setNodeRef} style={style} className={`bg-white rounded-xl p-5 border border-slate-200 shadow-sm relative ${isDragging ? 'ring-2 ring-blue-500 shadow-xl' : ''}`}>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
        <div className="space-y-1">
          ...
          <select {...register(`addresses.${index}.label`)} className="w-full p-2.5 bg-slate-50 border border-slate-200 rounded-lg text-sm text-slate-900 outline-none focus:border-blue-500 transition-colors">
        ...
        <div className="space-y-1">
          ...
          <input {...register(`addresses.${index}.street`)} placeholder="e.g. Business Bay" className={`w-full p-2.5 bg-slate-50 border ${errors.addresses?.[index]?.street ? 'border-red-300 focus:border-red-500' : 'border-slate-200 focus:border-blue-500'} rounded-lg text-sm text-slate-900 outline-none transition-colors`} />
        ...
        <div className="space-y-1">
          ...
          <select {...register(`addresses.${index}.city`)} className="w-full p-2.5 bg-slate-50 border border-slate-200 rounded-lg text-sm text-slate-900 outline-none focus:border-blue-500 transition-colors">
        ...
        <div className="space-y-1">
          ...
          <select {...register(`addresses.${index}.country`)} className="w-full p-2.5 bg-slate-50 border border-slate-200 rounded-lg text-sm text-slate-900 outline-none focus:border-blue-500 transition-colors">
            <option value="United Arab Emirates">United Arab Emirates</option>
          </select>
        </div>
        <div className="md:col-span-2 pt-2">
          <label className="flex items-center gap-2 cursor-pointer">
            <input type="checkbox" {...register(`addresses.${index}.isDefault`)} className="w-4 h-4 rounded border-slate-300 text-blue-600 focus:ring-blue-500" />

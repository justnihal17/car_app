      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-2">
          {order.status === 'Pending' && (
            <button className="px-4 py-2 bg-red-600 hover:bg-red-500 text-white font-medium rounded-lg text-sm shadow-lg shadow-red-600/10 transition-all flex items-center gap-2">
            ...
            <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-6 flex flex-col justify-between">
              ...
              <div className="pt-4 border-t border-slate-100">
                <div className="flex justify-between items-center mb-2">
                  ...
                  <span className="text-xl font-bold text-red-600">{order.amount}</span>
                </div>
              ...
              {TIMELINE.map((step, idx) => (
                <div key={idx} className="flex gap-4 relative">
                  {idx !== TIMELINE.length - 1 && (
                    <div className={`absolute left-3 top-8 w-px h-[calc(100%-8px)] ${step.done && TIMELINE[idx+1]?.done ? 'bg-red-600' : 'bg-slate-200'}`}></div>
                  )}
                  <div className="relative z-10 flex flex-col items-center mt-1">
                    <div className={`w-6 h-6 rounded-full flex items-center justify-center border-2 
                      ${step.current ? 'bg-white border-red-600 text-red-600' : 
                        step.done ? 'bg-red-600 border-red-600 text-white' : 
                        'bg-white border-slate-200 text-slate-400'}
                    `}>
                      {step.done && !step.current ? <CheckCircle className="w-3.5 h-3.5" /> : 
                       step.current ? <div className="w-2 h-2 rounded-full bg-red-600 animate-pulse" /> : null}
                    </div>
                  </div>
                  <div className="pb-6">
                    <div className={`text-sm ${step.current ? 'text-red-600 font-bold' : step.done ? 'text-slate-800 font-semibold' : 'text-slate-400'}`}>
                      {step.status}
                    </div>
                    {step.time && (
            ...
            <div className="flex items-center gap-4 mb-4">
              ...
              <div>
                ...
                <div className="text-xs text-red-600 font-semibold flex items-center gap-1 mt-0.5">
                  <Award className="w-3 h-3 text-red-500" /> {order.customer.membership}
                </div>
              </div>
            </div>
          ...
          <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-5">
            ...
            {order.driver ? (
              ...
              <div className="text-center py-6 bg-slate-50 rounded-lg border border-slate-100 p-4">
                ...
                <button className="px-4 py-2 bg-red-50 hover:bg-red-100 text-red-600 border border-blue-200 font-bold rounded-lg transition-colors text-sm w-full">
                  Assign Manually
                </button>
              </div>
            )}

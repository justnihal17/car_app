      ...
      >
        <div className={`flex items-center gap-3 overflow-hidden w-full ${collapsed ? 'justify-center' : 'justify-center px-2'}`}>
          <SafeImage 
            src="/logo.png" 
            alt="Stylein Logo" 
            className={`object-contain transition-all duration-300 mix-blend-multiply brightness-[1.05] contrast-[1.1] shrink-0 ${collapsed ? 'h-12' : 'h-20'}`}
          />
      ...
      <div className="flex-1 overflow-y-auto py-4 2xl:py-6 px-2 2xl:px-3 flex flex-col gap-1 2xl:gap-1.5 [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar:none]">
        ...
        {BOTTOM_MENU.map((item) => {
          ...
          return (
            ...
            >
              <Icon className={`w-4.5 h-4.5 2xl:w-5 2xl:h-5 shrink-0 ${isActive ? 'text-red-600' : (item.id === 'logout' ? 'text-red-500' : 'text-slate-700')}`} />
              {!collapsed && <span className="text-[13px] 2xl:text-sm tracking-tight">{item.label}</span>}
            </button>
          );
        })}

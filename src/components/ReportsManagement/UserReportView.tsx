...
export function UserReportView() {
  ...
  const summaryCards = [
    { 
      ...
      value: summary.totalCustomers ?? summary.total ?? summary.totalCount ?? userRecords.length ?? 0, 
      icon: Users, 
    ...
    { 
      ...
      value: summary.activeCustomers ?? summary.active ?? (userRecords.filter(u => u.active || u.status === 'Active').length || 0), 
      icon: Activity, 
      color: 'text-slate-700', 
      bg: 'bg-slate-50 border-slate-200/80' 
    },
    { 
      ...
      value: summary.customersWithOrders ?? summary.withOrders ?? summary.orderedCustomers ?? (userRecords.filter(u => (u.ordersCount || 0) > 0).length || 0), 
      icon: ShoppingBag, 
      color: 'text-slate-700', 
      bg: 'bg-slate-50 border-slate-200/80' 
    },
    { 
      ...
      value: summary.customersWithoutOrders ?? summary.withoutOrders ?? summary.nonOrderedCustomers ?? (userRecords.filter(u => !u.ordersCount).length || 0), 
      icon: UserX, 
      color: 'text-slate-700', 
      bg: 'bg-slate-50 border-slate-200/80' 
    },
  ...
  const parseDistribution = (rawList: any, nameKey: string = 'name') => {
    ...
    return rawList.map((item: any) => {
      let rawName = item.name ?? item[nameKey] ?? item.method ?? item.label ?? item._id ?? item.city ?? item.state ?? item.brand ?? item.model ?? item.type ?? '';
  ...
  const getDistributionFor = (reportArray: any[], recordField: string, fallbackNameKey: string = 'name') => {
    ...
    userRecords.forEach((u: any) => {
      let val: any = '';
      if (recordField === 'city') {
        val = u.city || u.addresses?.[0]?.city || u.addresses?.find((a: any) => a.city)?.city;
      } else if (recordField === 'state') {
        val = u.emirate || u.state || u.vehicles?.[0]?.stateId?.name || u.vehicles?.[0]?.state?.name || u.addresses?.[0]?.state;
      } else if (recordField === 'vehicleBrand') {
        val = u.brand?.name || u.brand || u.vehicles?.[0]?.makeId?.name || u.vehicles?.[0]?.make?.name || u.vehicles?.[0]?.brand?.name;
      } else if (recordField === 'vehicleModel') {
        val = u.model?.name || u.model || u.vehicles?.[0]?.modelId?.name || u.vehicles?.[0]?.model?.name;
      } else if (recordField === 'fuelType') {
        val = u.fuelType?.name || u.fuelType || u.vehicles?.[0]?.fuelTypeId?.name || u.vehicles?.[0]?.fuelType?.name;
      } else if (recordField === 'paymentMethod') {
        val = u.paymentMethod || u.payment;
      } else {
        val = u[recordField];
      }
  ...
  const byPaymentData = useMemo(() => getDistributionFor(
    ...
    customerData.byPaymentMethod || 
    reportData?.orders?.byPaymentMethod || 
    reportData?.byPaymentMethod || 
    [], 
    'paymentMethod',
    'method'
  ), [customerData, reportData, userRecords]);
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3">
        <div>
          <div className="flex items-center gap-1.5 text-[11px] font-medium text-slate-500 uppercase tracking-wider mb-1">
            ...
            <ChevronDown className="w-3 h-3 -rotate-90 text-slate-400" />
            <span className="text-red-600 font-semibold">User Report</span>
          </div>
          <h1 className="text-sm sm:text-base font-semibold text-slate-900 tracking-tight flex items-center gap-2">
            <Users className="w-4 h-4 text-red-600" /> User Report
          </h1>
        </div>
